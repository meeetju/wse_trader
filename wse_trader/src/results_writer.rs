...
impl Output for ConsolePrinter {
    fn write(&self, companies_list: Vec<Company>) -> Result<(), Box<dyn Error>> {
        ...
        for company in companies_list {
            println!("{}, {}, {}, {}, {}, {}, {}, {}", company.name, company.ticker, company.altman, company.f_score.to_string(), company.pe.to_string(), company.roe.to_string(), company.p_bv.to_string(), company.p_bvg.to_string());
        }
        Ok(())
    }
}